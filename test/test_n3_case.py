import os
import tempfile
from rdflib import logger, RDF, RDFS, URIRef, BNode, Variable
...
store = "SQLiteLSM"
configString = os.path.join(tempfile.gettempdir(), f"test_{store.lower()}")
...
def test_n3_store():
    ...
    if configString:
        logger.debug(f"configString = {configString}")
