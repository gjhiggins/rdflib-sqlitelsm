...
def test_guess_format_for_parse(get_graph):
    ...
    graph.parse("test/consistent_test_data/anons-01.nt")
    # RDF/XML
    graph.parse("test/consistent_test_data/test001.rdf")  # XML
    # bad filename but set format
    graph.parse("test/consistent_test_data/test001.borked", format="xml")
...
def test_parse_file_uri(get_graph):
    ...
    graph.parse(
        Path("./test/consistent_test_data/simple-04.nt").absolute().as_uri()
    )
...
def test_transitive(get_graph):
    ...
    assert set(
        graph.transitive_subjects(predicate=parent, object=mom_of_mom)
    ) == {
        mom_of_mom,
        mom,
        person,
